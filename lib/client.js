...
let openDB;
let openDBPromise; // For async setup of IndexedDBShim
let isFileSystem = false;

if (typeof window === 'undefined') {
  // Node.js environment - support multiple storage backends
  const cliMode = process.env.INDEXCP_CLI_MODE;
  
  if (cliMode === 'false') {
    // Mode: fake-indexeddb (in-memory, for testing)
    require('fake-indexeddb/auto');
    const { openDB: idbOpenDB } = require('idb');
    openDB = idbOpenDB;
  } else if (cliMode === 'json') {
    // Mode: JSON file-based (legacy, for backwards compatibility)
    const { openFileSystemDB } = require('./filesystem-db');
    openDB = openFileSystemDB;
    isFileSystem = true;
  } else {
    // Default mode: IndexedDBShim + WebSQL (SQLite) for persistent storage
    const { setupIndexedDBShim } = require('./indexeddb-shim');
    // Store the promise for async setup
    openDBPromise = setupIndexedDBShim();
    // Create a wrapper that awaits the setup
    openDB = async (...args) => {
      const actualOpenDB = await openDBPromise;
      return actualOpenDB(...args);
    };
  }
} else {
  // Browser environment
