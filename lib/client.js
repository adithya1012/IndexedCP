...
if (typeof window === 'undefined') {
  // Node.js environment
  // Storage modes:
  // - 'test': Use fake-indexeddb (in-memory, no persistence) for testing
  // - 'sqlite': Use SQLite backend (persistent, production)
  // - 'filesystem': Use legacy JSON files (deprecated, for backward compatibility)
  // - Default: Use SQLite backend
  const storageMode = process.env.INDEXEDCP_STORAGE_MODE || 'sqlite';

  if (storageMode === 'test') {
    // Test mode: Use fake-indexeddb (in-memory, no persistence)
    require('fake-indexeddb/auto');
    const { openDB: idbOpenDB } = require('idb');
    openDB = idbOpenDB;
  } else if (storageMode === 'filesystem') {
    // Legacy mode: Use JSON file storage (deprecated)
    const { openFileSystemDB } = require('./filesystem-db');
    openDB = openFileSystemDB;
    isFileSystem = true;
  } else {
    // Production mode (default): Use SQLite backend
    const { openSQLiteDB } = require('./sqlite-db');
    openDB = openSQLiteDB;
    isSQLite = true;
  }
} else {
...
class IndexedCPClient {
  constructor(options = {}) {
    ...
    if (this.encryption) {
      ...
      // Select encrypted DB based on storage mode
      const storageMode = process.env.INDEXEDCP_STORAGE_MODE || 'sqlite';
      let encryptedDBModule;
      
      if (storageMode === 'test') {
        // For testing, use fake-indexeddb with in-memory storage
        // This is handled by setting up fake-indexeddb/auto above
        // We still need the encrypted-db for the API, but data won't persist
        encryptedDBModule = require('./encrypted-db');
      } else if (storageMode === 'filesystem') {
        // Legacy JSON file storage
        encryptedDBModule = require('./encrypted-db');
      } else {
        // SQLite backend (default for production)
        encryptedDBModule = require('./sqlite-encrypted-db');
      }
      
      this.cryptoUtils = cryptoUtils;
      this.encryptedDB = encryptedDBModule.openSQLiteEncryptedDB || encryptedDBModule.openEncryptedDB;
  ...
  async fetchPublicKey() {
    ...
    this.logger.info(`✓ Fetched and cached server public key (kid: ${publicKeyInfo.kid})`);
  ...
  async startStream(fileName) {
    ...
    this.logger.info(`✓ Started encrypted stream: ${sessionId} for ${fileName}`);
    return sessionId;
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      readStream.on('end', async () => {
        try {
          ...
          this.logger.info(`File ${fileName} added to buffer with ${chunkIndex} chunks`);
          resolve(chunkIndex);
        } catch (error) {
          reject(error);
        }
      ...
      return new Promise((resolve, reject) => {
        ...
        readStream.on('end', async () => {
          try {
            ...
            this.logger.info(`✓ File ${fileName} encrypted and buffered (${seq} packets)`);
            resolve(sessionId);
          } catch (error) {
  ...
  async uploadBufferedFiles(serverUrl) {
    ...
    this.logger.info(`Grouped into ${Object.keys(fileGroups).length} files:`, Object.keys(fileGroups));
  ...
  async uploadFileChunks(serverUrl, fileName, chunks, db, apiKey) {
    ...
    if (serverFilename !== require('path').basename(fileName)) {
      this.logger.info(`Upload complete for ${fileName} -> Server saved as: ${serverFilename}`);
    } else {
  ...
  async uploadEncryptedFiles(serverUrl) {
    ...
    this.logger.info(`Found ${pendingPackets.length} encrypted packets to upload`);
  ...
  async uploadSession(serverUrl, sessionId, sessionPackets, db, apiKey) {
    ...
    this.logger.info(`Uploading ${session.fileName} (${sessionPackets.length} encrypted packets)...`);
    ...
    try {
      if (contentType && contentType.includes('application/json')) {
        ...
        if (responseData.actualFilename && responseData.actualFilename !== fileName && responseData.actualFilename !== require('path').basename(fileName)) {
          this.logger.info(`Server used filename: ${responseData.actualFilename} (client sent: ${fileName})`);
        }
      } else {
  ...
  startUploadBackground(serverUrl, options = {}) {
    ...
    this.logger.info(`🚀 Starting background upload (checking every ${checkInterval}ms)`);
  ...
  async _processUnencryptedBackgroundUpload(serverUrl, db, now) {
    ...
    allRecords.forEach(record => {
      ...
      if (record.retryMetadata.retryCount >= this.maxRetries) {
        this.logger.warn(`⚠ Max retries (${this.maxRetries}) reached for chunk ${record.id}`);
        return;
      }
    ...
    this.logger.info(`📤 Background upload: ${fileCount} file(s) with pending chunks`);
  ...
  async _uploadFileChunksWithRetry(serverUrl, fileName, chunks, db, apiKey, now) {
    ...
    for (const chunk of chunks) {
      try {
        ...
        this.logger.warn(`⚠ Upload failed for ${fileName} chunk ${chunk.chunkIndex} (retry ${chunk.retryMetadata.retryCount}/${this.maxRetries === Infinity ? '∞' : this.maxRetries}). Next retry in ${Math.round(delay/1000)}s`);
    ...
    this.logger.info(`✓ Successfully uploaded ${fileName} (${successCount} chunks)`);
  }
  ...
  async _processEncryptedBackgroundUpload(serverUrl, db, now) {
    ...
    const retryablePackets = packets.filter(packet => {
      ...
      if (packet.retryMetadata.retryCount >= this.maxRetries) {
        this.logger.warn(`⚠ Max retries (${this.maxRetries}) reached for packet ${packet.id}`);
        return false;
      }
    ...
    this.logger.info(`📤 Background upload: ${Object.keys(sessionGroups).length} session(s) with ${retryablePackets.length} pending packets`);
  ...
  async _uploadSessionWithRetry(serverUrl, sessionId, sessionPackets, db, apiKey, now) {
    ...
    for (const packet of sessionPackets) {
      try {
        ...
        this.logger.warn(`⚠ Upload failed for ${session.fileName} packet ${packet.seq} (retry ${packet.retryMetadata.retryCount}/${this.maxRetries === Infinity ? '∞' : this.maxRetries}). Next retry in ${Math.round(delay/1000)}s`);
    ...
    if (errors.length === 0) {
      this.sessionKeys.delete(sessionId);
      this.sessionSeqCounters.delete(sessionId);
      this.logger.info(`✓ Successfully uploaded ${session.fileName} (${successCount} packets)`);
    } else {
